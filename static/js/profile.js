...
function applyProfileUI(data) {
    ...
    document.getElementById("user-info").innerHTML =
        `${headerAvatarHtml(data)}
         <span class="font-semibold text-white">${safeShortName}</span>
         &nbsp;|&nbsp; 🪙 <span class="text-yellow-400 font-bold">${data.total_points.toLocaleString()}</span>đ`;
...
function headerAvatarHtml({ avatar_url, avatar_color, initials }) {
    ...
    if (avatarSrc) {
        return `<img src="${avatarSrc}" style="width:20px;height:20px;border-radius:50%;object-fit:cover;
                     border:1px solid #34d399;vertical-align:middle;margin-right:5px;">`;
    }
...
function renderBets(bets, listEl) {
    if (bets.length === 0) {
        listEl.innerHTML = `
            <div class="text-center py-12 text-gray-500 text-sm">
                ...
                <a href="/" class="text-emerald-400 text-sm mt-2 inline-block hover:underline">Xem trận đấu ngay →</a>
            </div>`;
        return;
    }
    ...
    listEl.innerHTML = bets.map(b => {
        ...
        const scoreOrVs    = isFinished
            ? `<span class="font-black text-white">${b.home_score} - ${b.away_score}</span>`
            : `<span class="text-gray-500 font-bold">vs</span>`;

        return `
        <div>
            <div class="bet-card bg-gray-800 border border-gray-700 rounded-xl p-3 flex items-center gap-3"
                 id="card-${b.bet_id}" onclick="toggleDetail(${b.bet_id})">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-1 text-sm font-semibold text-white mb-1 truncate">
                        ${homeIconHtml}${homeTeam}
                        <span class="mx-1 text-gray-500 text-xs font-normal">${scoreOrVs}</span>
                        ${awayTeam}${awayIconHtml}
                    </div>
                    <div class="flex items-center gap-2 text-xs text-gray-400">
                        <span class="text-gray-500">Chọn:</span>
                        <span class="font-medium text-gray-200">${choiceLabel}</span>
                        <span class="text-gray-600">•</span>
                        <span class="text-yellow-500 font-semibold">${b.stake.toLocaleString()} điểm</span>
                    </div>
                </div>
                <div class="flex flex-col items-end gap-1 flex-shrink-0">
                    ${badgeHtml}
                    <svg id="arrow-${b.bet_id}" class="w-4 h-4 text-gray-500 transform transition-transform duration-200"
                         fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                    </svg>
                </div>
            </div>
            <div id="detail-${b.bet_id}" class="hidden detail-panel border-x border-b border-gray-700 rounded-b-xl px-4 pt-3 pb-4 -mt-1 bg-gray-800">
                ${renderDetail(b)}
            </div>
        </div>`;
    }).join("");
...
function renderDetail(b) {
    ...
    const resultHtml = isFinished
        ? isRefunded
            ? `<div class="flex justify-between"><span class="text-gray-400">Kết quả</span>
               <span class="font-semibold text-sky-400">Hoàn ${b.stake.toLocaleString()} điểm</span></div>`
            : isWin
            ? `<div class="flex justify-between"><span class="text-gray-400">Điểm nhận về</span>
               <span class="font-black text-emerald-400 text-base">+${b.points_earned.toLocaleString()} điểm</span></div>`
            : `<div class="flex justify-between"><span class="text-gray-400">Kết quả</span>
               <span class="font-semibold text-red-400">Mất ${b.stake.toLocaleString()} điểm</span></div>`
        : `<div class="flex justify-between"><span class="text-gray-400">Kết quả</span>
           <span class="text-violet-400 font-semibold">Đang chờ kết quả...</span></div>`;

    return `<div class="space-y-2 text-sm">
        <div class="flex justify-between"><span class="text-gray-400">Trận đấu</span><span class="font-semibold">${homeTeam} vs ${awayTeam}</span></div>
        <div class="flex justify-between"><span class="text-gray-400">Kèo chấp</span><span class="text-gray-300">${hcSign}${b.handicap}</span></div>
        ${isFinished ? `<div class="flex justify-between"><span class="text-gray-400">Tỉ số</span><span class="font-bold text-white">${b.home_score} - ${b.away_score}</span></div>` : ""}
        <div class="flex justify-between"><span class="text-gray-400">Lựa chọn</span><span class="font-semibold text-emerald-300">${choiceLabel}</span></div>
        <div class="flex justify-between"><span class="text-gray-400">Số điểm đặt</span><span class="text-yellow-400 font-semibold">${b.stake.toLocaleString()} điểm</span></div>
        <div class="border-t border-gray-700 pt-2 mt-2">${resultHtml}</div>
        <div class="text-xs text-gray-600 pt-1">🗓 Trận: ${fmt(b.start_time)} &nbsp;•&nbsp; ⏱ Đặt: ${fmt(b.created_at)}</div>
    </div>`;
}
